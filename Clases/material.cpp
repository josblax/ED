...
public:
	// Constructor
	Material(string id,string titulo,string genero,string tipo,double duracion,vector<string> actores,vector<string> directores,string region,
		int cantidad,bool referenciaLimitada) :id(id),titulo(titulo),genero(genero),tipo(tipo),duracion(duracion),actores(actores),directores(directores),region(region),cantidad(cantidad),referenciaLimitada(referenciaLimitada)
		{}
	//getters
